package com.locally.backend.controller.wallet;
...
public class WalletController {
    ...
    public ResponseEntity<AppResponse<TransactionResponse>> addMoney(@RequestHeader("Authorization") String authHeader, @Valid @RequestBody AddMoneyRequest addMoneyRequest) {

        String token = authHeader.substring(7);
        String email = jwtUtil.retrieveSubject(token);

        addMoneyRequest.setEmail(email);

        AppResponse<TransactionResponse> transactionResponse = walletService.addMoney(addMoneyRequest);
    ...
    public ResponseEntity<AppResponse<Object>> getWalletBalance(@RequestHeader("Authorization") String authHeader) {
        ...
        Map<String, Object> responseData = Map.of(
                "userId", wallet.getData().getUserId(),
